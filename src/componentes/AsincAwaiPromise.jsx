...
function AsincAwaiPromise() {
  ...
  useEffect(() => {
    ...
    const fetchComentarios = async () => {
      ...
      try {
        ...
        // Esperamos el JSON de la respuesta
  ...
  // Mostramos los comentarios en tarjetas
  return (
    <div className="p-5 flex flex-wrap gap-4 justify-center">
      {comentarios.map((comentario) => (
        <div key={comentario.id} className="w-96 bg-white shadow-lg rounded-lg flex p-4">
          ...
          {/* Aquí podrías agregar más contenido o imágenes si lo deseas */}
        </div>
      ))}
    </div>
  );
...
export default AsincAwaiPromise;
